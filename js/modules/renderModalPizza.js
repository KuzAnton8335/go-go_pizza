export const renderModalPizza = ({ id, images, name, price, toppings }) => {
	...
	title.textContent = name.ru[0].toUpperCase() + name.ru.slice(1).toLowerCase();
	...
	toppingElement.textContent = toppings.ru[0].toUpperCase() + toppings.ru.slice(1).toLowerCase();
	...
	// ?to-do продолжить верстать модальное окно для карточики пиццы....
